...
일반 이메일 회원가입의 봇/스팸 가입 방지를 위해 Cloudflare Turnstile을 사용합니다.
...
Turnstile은 클라이언트 위젯만 붙이면 완료가 아닙니다. 회원 생성 전에 서버에서 반드시 Cloudflare Siteverify API로 token을 검증해야 합니다.
...
- `response`: 회원가입 요청에서 전달된 `turnstileToken`
- `remoteip`: 가능하면 요청 IP

성공 조건:

- Siteverify 응답의 `success`가 `true`일 때만 회원 생성을 허용합니다.
- 실패하면 회원 생성 요청을 거부하고 일반화된 오류 메시지를 반환합니다.
