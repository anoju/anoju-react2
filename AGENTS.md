...
* **헤더 알림 버튼:** 로그인한 사용자에게는 메인 헤더 우측에 알림함으로 이동하는 `IconButton`을 제공하고, 읽지 않은 알림이 있으면 접근성 이름과 시각 배지를 함께 갱신합니다. 알림 버튼 노출 여부는 라우트/레이아웃 설정에서 화면별로 제어합니다.
