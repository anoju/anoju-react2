...
* **하단 플로팅 버튼:** 하단 플로팅 버튼은 모든 화면에 배치하지 않고, `홈` 화면과 플로팅 메뉴 항목으로 직접 이동하는 주요 화면에서 기본 노출합니다. 기본 메뉴는 `전체메뉴`, `playground`, `Snaps`, `마이페이지`로 구성하며, 기존 `홈` 항목은 제외합니다. 자유게시판 목록/상세/작성/수정, Pics 피드/상세/작성 등 콘텐츠 소비/작성 중심의 하위 화면에서는 기본 미노출하되, 라우트/레이아웃 설정에서 화면별로 재정의할 수 있어야 합니다.
* **플로팅 메뉴 콘텐츠 진입:** `playground`는 커뮤니티형 게시판 묶음의 1뎁스 서브 메인으로 이동합니다. `Snaps`는 시각 콘텐츠 1뎁스 서브 메인으로 이동합니다. 플로팅 메뉴에서 `자유게시판`이나 `Pics` 같은 2뎁스 화면으로 바로 이동하지 않습니다.
...
* **게시판 메뉴 구조:** 커뮤니티형 게시판 묶음은 1뎁스 `playground`, 하위 2뎁스 첫 메뉴는 `자유게시판`, 추가 2뎁스 메뉴는 `디바이스정보`로 구성합니다. 이미지/소셜 피드 묶음은 1뎁스 `Snaps`, 하위 2뎁스 첫 메뉴는 `Pics`로 구성합니다. `playground`와 `Snaps`는 추후 2뎁스 메뉴 확장을 전제로 각각 서브 메인 화면을 가져야 합니다.
* **콘텐츠 시간 표기:** 자유게시판, Pics, 댓글, 마이페이지 활동 목록의 사용자 노출 시간은 기본적으로 `5분 전`, `2시간 전` 같은 상대 시간으로 통일합니다. 오래된 콘텐츠처럼 절대 날짜가 필요한 경우에만 보조 정보 또는 title 속성으로 절대 시간을 제공합니다.
* **내 게시물 보기:** 자유게시판, Pics를 포함한 모든 게시판/갤러리 목록 화면은 `내 게시물 보기` 기능을 제공해야 합니다. 이 기능은 현재 로그인한 사용자가 작성한 게시물만 필터링하며, 검색/정렬/태그와 동일하게 URL query와 동기화하고 변경 시 `DataList`의 페이지/커서 상태를 초기화합니다. 미로그인 사용자가 진입하면 로그인 화면으로 이동시키고 로그인 후 원래 목록으로 복귀할 수 있어야 합니다. 마이페이지의 `내가 작성한 글`은 전체 작성글 모아보기이고, 각 게시판/갤러리의 `내 게시물 보기`는 해당 목록 안에서 동작하는 필터로 구분합니다.
* **자유게시판 UX:** `자유게시판`은 일반 커뮤니티 게시판처럼 목록, 상세, 작성, 수정, 삭제, 댓글을 제공하며, 리치 텍스트 에디터를 통해 글 중간 이미지 삽입을 지원합니다. 이미지 외 일반 파일은 별도 파일첨부 목록으로 관리합니다. 목록은 모바일 중심의 단순 리스트 형태로 만들고 테이블형 레이아웃은 사용하지 않습니다. 작성/댓글 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다. 수정은 작성자 본인과 관리자만 가능하며 `/playground/free-board/:postId/edit` 경로에서 제목과 본문을 수정합니다. 목록과 상세의 회원 이름 좌측에는 프로필 이미지를 원형 아바타로 함께 노출합니다. 상세 본문 하단에는 `좋아요`, `싫어요`, `공유하기` 액션을 함께 배치합니다.
* **디바이스정보 UX:** `디바이스정보`는 일반 게시판이 아니라 모바일 디바이스 웹 해상도 데이터 목록/상세/작성 중심으로 제공합니다. 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다. 수정은 작성자 본인과 관리자만 가능하며 `/playground/device/:deviceReportId/edit` 경로에서 제조사, 모델, 디스플레이설정, 추가설명을 수정합니다. 제조사와 모델은 이전 저장값 선택 + 직접입력 방식을 지원하고, 스크린 사이즈/윈도우 사이즈/ratio/스크린방향/userAgent는 자동 입력합니다. 윈도우 사이즈는 작성 중 resize와 orientationchange 때 관측값을 갱신하여 최소/최대값을 저장합니다. 디스플레이설정은 Android 기준 좌측 첫 단계를 `1`로 보는 숫자 입력이며, iPhone/iPad/iOS 계열은 `0`으로 자동 입력하고 수정할 수 없습니다. 목록은 제조사, 모델, 윈도우 사이즈, ratio, 스크린방향, 디스플레이설정을 보여주고 제조사/모델 필터와 검색 기능을 URL query와 동기화합니다. 상세의 일반 조회 액션은 공유하기만 제공하고, 작성자 본인과 관리자에게만 수정/삭제 액션을 추가로 노출합니다.
* **댓글 UX:** 댓글 항목에는 작성자 이름 좌측 프로필 이미지, 내용, 상대 시간과 함께 `좋아요`, `싫어요` 액션을 제공합니다. 게시글 작성자의 댓글은 `글쓴이` 배지와 하이라이트로 구분합니다. 댓글은 대댓글 작성을 지원하며 `parentComment` 관계로 묶어 들여쓰기 형태로 표시합니다. 댓글 반응은 로그인 및 이메일 인증 완료 회원만 사용할 수 있고, 같은 대상에 대한 좋아요/싫어요는 상호 배타적으로 처리합니다. 댓글 수정과 삭제는 작성자 본인 및 관리자만 사용할 수 있으며, 삭제 전에는 반드시 `Confirm`을 사용하고 soft delete로 숨김 처리합니다.
* **Pics UX:** `Pics`는 인스타그램 같은 소셜 피드 UI를 기준으로 작성자 이름 좌측 프로필 이미지, 이미지, 캡션, 좋아요, 싫어요, 댓글, 스크랩, 공유 액션을 제공합니다. 작성 화면에서는 사용자 입력 제목을 받지 않고 이미지 업로드/대표 이미지 지정 후 가장 아래에 캡션 작성 영역을 둡니다. 수정은 작성자 본인과 관리자만 가능하며 `/snaps/pics/:postId/edit` 경로에서 캡션을 수정하고 기존 이미지는 확인용으로 보여줍니다. 이미지 필터/보정/편집 기능은 제공하지 않고 업로드, 순서 관리, 대표 이미지, 확대 보기 중심으로 제한합니다. 상세 본문 하단에는 `좋아요`, `싫어요`, `공유하기` 액션을 함께 배치합니다.
* **마이페이지 구조:** `/my-page`는 마이페이지 1뎁스 서브 메인으로 사용하고, `/my-page/profile`, `/my-page/posts`, `/my-page/comments`처럼 기능별 화면을 분리합니다. 회원정보 수정 화면에서는 프로필 이미지를 등록/수정/삭제할 수 있어야 하며, 업로드 시 정사각형 크롭 UI를 제공합니다. 플로팅 메뉴의 `마이페이지`는 서브 메인으로 이동하며, 기능별 하위 화면은 기본적으로 플로팅 메뉴를 노출하지 않습니다.
