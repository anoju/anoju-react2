...
* **자유게시판 UX:** `자유게시판`은 일반 커뮤니티 게시판처럼 목록, 상세, 작성, 수정, 삭제, 댓글을 제공하며, 리치 텍스트 에디터를 통해 글 중간 이미지 삽입을 지원합니다. 이미지 외 일반 파일은 별도 파일첨부 목록으로 관리합니다. 목록은 모바일 중심의 단순 리스트 형태로 만들고 테이블형 레이아웃은 사용하지 않습니다. 작성/댓글 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다.
* **Pics UX:** `Pics`는 인스타그램 같은 소셜 피드 UI를 기준으로 작성자, 이미지, 캡션, 좋아요, 댓글, 스크랩, 공유 액션을 제공합니다. 작성 화면에서는 사용자 입력 제목을 받지 않고 이미지 업로드/대표 이미지 지정 후 가장 아래에 캡션 작성 영역을 둡니다. 이미지 필터/보정/편집 기능은 제공하지 않고 업로드, 순서 관리, 대표 이미지, 확대 보기 중심으로 제한합니다.
