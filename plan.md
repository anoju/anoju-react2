...
- **메뉴별 그룹 규칙:** 화면이 늘어나는 메뉴/도메인은 상위 메뉴 폴더로 묶고, 하위 화면은 `src/pages/메뉴명/화면명/Index.tsx` 구조를 사용합니다. 예: `src/pages/Lounge/FreeBoard/Index.tsx`, `src/pages/Snaps/Pics/Detail/Index.tsx`, `src/pages/MyPage/Profile/Index.tsx`
...
- **프로필 시스템:**
  ...
  - **닉네임 유니크 정책:** 닉네임은 사용자 노출명과 `@nickname` 태그 식별자로 사용하므로 필수값으로 관리합니다. 허용 문자는 한글, 영문 소문자, 숫자, `_`이며 2~20자로 제한합니다. 회원가입과 마이페이지 수정 시 클라이언트 중복 확인을 수행하고, PocketBase `users.nickname`에도 unique 제약을 둬 동시 저장 충돌을 서버에서 최종 차단합니다. 별도 `name` 사용자 필드는 사용하지 않고 제거합니다.
...
- **마이페이지 서브 메인:**
  ...
  - 하위 기능은 `/my-page/profile`, `/my-page/posts`, `/my-page/comments`, `/my-page/notifications`처럼 기능별 화면으로 분리합니다.
  - `내 정보` 화면은 프로필 수정, 프로필 이미지 등록/수정/삭제, 이메일 인증, 소셜 로그인 연결/해제를 관리합니다.
  - `내가 작성한 글` 화면은 자유게시판, Pics, picLog에 작성한 글을 모아 보여줍니다.
  - `내가 작성한 댓글` 화면은 사용자가 작성한 댓글과 연결된 게시글 이동을 제공합니다.
  - `알림함` 화면은 내 글 댓글, 내 댓글의 대댓글, 사용자 태그, picLog 초대/순서 변경 요청처럼 사용자 확인이 필요한 알림을 모아 보여줍니다.
  - 플로팅 메뉴에서 `마이페이지`를 누르면 `/my-page` 서브 메인으로 이동하며, 기능별 하위 화면은 기본적으로 플로팅 메뉴를 노출하지 않습니다.
- **알림함:**
  - 기본 경로는 `/my-page/notifications`로 두고, 로그인한 사용자만 접근할 수 있는 보호 화면으로 관리합니다.
  - 알림 발생 대상은 `내 글에 댓글 또는 대댓글이 달린 경우`, `내 댓글에 대댓글이 달린 경우`, `누군가 댓글/본문/picLog 메모에서 나를 태그한 경우`, `picLog 초대 또는 참여자 순서 변경 요청이 온 경우`, `관리자 공지성 개인 알림이 필요한 경우`를 1차 범위로 둡니다.
  - 사용자가 자기 글이나 자기 댓글에 직접 남긴 액션은 기본 알림에서 제외하여 불필요한 자기 알림을 만들지 않습니다.
  - 알림 항목은 유형, 발생 사용자, 대상 콘텐츠 요약, 연결 경로, 읽음 여부, 생성 시간을 포함하고, 시간은 다른 활동 목록과 동일하게 상대 시간으로 표시합니다.
  - 알림 목록은 최신순으로 제공하고, `전체`, `읽지 않음`, `댓글`, `태그`, `요청` 필터를 1차 후보로 둡니다. 필터는 URL query와 동기화하고 변경 시 목록 페이지/커서 상태를 초기화합니다.
  - 알림 항목을 누르면 연결된 게시글, 댓글 위치, picLog 챕터, 요청 처리 화면 등 맥락이 보존된 위치로 이동합니다. 이동 가능한 대상이 삭제되었거나 권한이 없으면 `ErrorState` 또는 `Toast`로 한글 안내를 제공합니다.
  - 읽음 처리는 항목 진입 시 개별 읽음 처리하고, 목록 상단에는 `모두 읽음` 액션을 제공합니다. 삭제 대신 읽음/숨김 상태를 우선 사용하고, 오래된 알림 정리는 서버 정책으로 별도 관리합니다.
  - 실시간 알림은 PocketBase `subscribe`를 우선 활용하되, 브라우저 푸시 알림은 PWA/권한/서버 푸시 정책이 확정된 뒤 2차 기능으로 분리합니다.
  - 메인 헤더 우측에는 알림 `IconButton`을 배치하고, 읽지 않은 알림이 있으면 작은 배지 또는 점 표시를 제공합니다. 로그인 전에는 알림 아이콘을 숨기거나 로그인 화면으로 이동시키는 정책 중 하나를 구현 전에 확정합니다.
  - 전체메뉴에는 `Members` 섹션 아래에 `알림함` 메뉴를 추가하고, 마이페이지 서브 메인에도 `알림함` 이동 메뉴를 제공합니다.

### 6.2 게시판 및 갤러리 (Board & Gallery - PocketBase 연동)

- **메뉴 구조:**
  - 커뮤니티형 게시판 묶음은 1뎁스 `lounge`로 관리합니다.
  - `lounge` 하위 2뎁스 첫 메뉴는 `자유게시판`으로 둡니다.
  - `lounge` 하위 2뎁스에는 모바일 웹 해상도 확인용 `디바이스정보` 게시판을 함께 둡니다.
  - 이미지/소셜 피드 묶음은 1뎁스 `Snaps`로 관리합니다.
  - `Snaps` 하위 2뎁스 첫 메뉴는 `Pics`로 둡니다.
  - `Snaps` 하위 2뎁스 추가 메뉴로 사진 기반 하루 기록 서비스 `picLog`를 둡니다.
  - `lounge`와 `Snaps`는 추후 2뎁스 메뉴가 계속 추가될 전제이므로 각각 서브 메인 화면을 제공합니다.
  - 1뎁스 서브 메인 화면은 하위 메뉴 카드/최근 콘텐츠/인기 콘텐츠/작성 진입점을 제공하되, 실제 게시글 목록 화면과 역할을 분리합니다.
  - 플로팅 메뉴의 `lounge`와 `Snaps` 항목은 각각 1뎁스 서브 메인 화면으로 이동해야 하며, 2뎁스 목록 또는 상세 화면으로 바로 이동하지 않습니다.
- **컨텐츠 레이아웃:**
  - **자유게시판:** 일반 커뮤니티 게시판 형태로 목록, 상세, 작성, 수정, 삭제, 댓글을 제공합니다.
  - **콘텐츠 시간 표기:** 자유게시판, Pics, picLog, 댓글, 마이페이지 활동 목록에서 사용자에게 노출하는 시간은 `방금 전`, `5분 전`, `2시간 전` 같은 상대 시간으로 통일합니다. 오래된 콘텐츠에는 상대 시간 대신 날짜를 표시하거나 title 속성으로 절대 시간을 보조 제공할 수 있습니다.
  - **내 게시물 보기:** 자유게시판, Pics, picLog를 포함한 모든 게시판/갤러리 목록에는 `내 게시물 보기` 기능을 제공합니다. 로그인한 사용자의 작성글만 현재 목록 안에서 필터링하며, 마이페이지의 `내가 작성한 글`처럼 전체 도메인을 모아 보여주는 화면과는 별도 기능으로 구분합니다. 미로그인 상태에서 사용하면 로그인 화면으로 이동하고, 로그인 후 기존 목록 경로와 query로 복귀할 수 있어야 합니다.
  - **자유게시판 목록:** 모바일 중심의 단순 리스트 형태로 구성합니다. 제목, 작성자 프로필 이미지, 작성자, 상대 작성시간, 댓글 수, 조회 수, 좋아요/싫어요 수를 빠르게 스캔할 수 있게 하되, 테이블형 레이아웃은 사용하지 않습니다.
  - **자유게시판 상세:** 작성자 닉네임 좌측에 프로필 이미지를 표시하고, 상세 본문 아래에 `좋아요`, `싫어요`, `공유하기` 액션을 같은 액션 그룹으로 배치합니다. 공유하기는 현재 게시글 URL을 기준으로 동작하며, 좋아요/싫어요는 로그인 및 이메일 인증 완료 회원만 사용할 수 있습니다.
  - **자유게시판 수정:** `/lounge/free-board/:postId/edit` 경로에서 제목과 본문을 수정합니다. 수정 권한은 작성자 본인과 관리자에게만 제공하며, 저장 성공 후 상세 화면으로 이동합니다.
  ...
  - **자유게시판 댓글:** 댓글과 대댓글을 지원하되, 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다. 댓글 항목에는 작성자 닉네임 좌측 프로필 이미지, 상대 시간, 좋아요, 싫어요 액션을 제공합니다.
  - **댓글 대댓글:** 자유게시판과 Pics 댓글은 `parentComment` 관계를 사용해 대댓글 작성을 지원합니다. 게시글 작성자가 남긴 댓글은 `글쓴이` 배지와 하이라이트로 구분하고, 대댓글은 부모 댓글 아래 들여쓰기 형태로 표시합니다.
  - **댓글 수정/삭제:** 자유게시판과 Pics 댓글은 작성자 본인과 관리자만 수정/삭제할 수 있습니다. 댓글 삭제는 soft delete로 처리하고 목록에서 숨기며, 삭제 전 `Confirm`을 사용합니다.
  - **디바이스정보:** 일반 게시판보다 디바이스 데이터 목록/상세/작성 중심으로 구성합니다. 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다.
  - **디바이스정보 작성 항목:** 제조사, 모델, 스크린 사이즈, 윈도우 사이즈, ratio, 스크린방향, userAgent, 디스플레이설정, 추가설명을 저장합니다. 제조사와 모델은 이전 저장값을 선택하거나 직접 입력할 수 있어야 하며, 모델 선택지는 제조사 선택값에 따라 좁힐 수 있습니다.
  - **디바이스정보 자동 입력:** 스크린 사이즈, 윈도우 사이즈, ratio, 스크린방향, userAgent는 브라우저 값을 기준으로 자동 입력합니다. 윈도우 사이즈는 작성 화면 진입 후 `resize`와 `orientationchange`에 반응하여 관측된 최소/최대 width, height를 함께 저장합니다.
  - **디바이스정보 디스플레이설정:** Android 등 설정의 화면 크기/표시 크기 단계 기준입니다. 설정에서 가장 왼쪽을 `1`로 보고 현재 단계를 입력합니다. iPhone나 iPad등 설정을 바꿀수 없을경우 `0`으로 입력해주세요.
  - **디바이스정보 목록:** 제조사, 모델, 윈도우 사이즈, ratio, 스크린방향, 디스플레이설정을 모바일 데이터 카드 형태로 보여줍니다. 제조사 필터, 모델 필터, 검색어는 URL query와 동기화하고 변경 시 `DataList` 페이지 상태를 초기화합니다.
  - **디바이스정보 상세:** 저장된 측정값과 추가설명을 읽기 쉬운 정의형 레이아웃으로 보여주고, 일반 조회자가 사용하는 소셜 기능은 공유하기만 제공합니다. 작성자 본인과 관리자에게만 수정/삭제 액션을 추가로 노출합니다.
  - **디바이스정보 수정:** `/lounge/device/:deviceReportId/edit` 경로에서 제조사, 모델, 스크린 사이즈, 윈도우 사이즈, ratio, 스크린방향, userAgent, 디스플레이설정, 추가설명을 수정합니다. 자동으로 등록된 측정값도 직접 입력할 수 있고, 윈도우 사이즈는 현재 브라우저 기준으로 다시 측정하는 액션을 제공합니다. 수정 권한은 작성자 본인과 관리자에게만 제공합니다.
  - **Pics:** 인스타그램 같은 소셜 피드 UI를 기준으로 이미지가 중심이 되는 카드형 피드를 제공합니다.
  - **Pics 피드:** 작성자 닉네임 좌측 프로필 이미지, 이미지, 캡션, 좋아요, 싫어요, 댓글, 스크랩, 공유 액션을 한 흐름에서 확인할 수 있게 구성합니다.
  - **Pics 작성:** 소셜 UI 기준으로 사용자 입력 제목은 받지 않고, 이미지 업로드/대표 이미지 지정 후 가장 아래에 캡션 작성 영역을 둡니다. DB의 필수 `title` 값은 캡션 일부를 기반으로 내부 생성합니다.
  - **Pics 수정:** `/snaps/pics/:postId/edit` 경로에서 캡션을 수정합니다. 기존 이미지는 확인용으로 보여주고, 이미지 교체/순서 변경/대표 이미지 변경은 이미지 레코드 삭제와 재정렬 정책을 별도 확정한 뒤 확장합니다. 수정 권한은 작성자 본인과 관리자에게만 제공합니다.
  ...
  - **picLog 컨셉:** `picLog`는 setlog의 짧은 순간 기록 감성을 사진 중심으로 재해석한 `하루 사진 로그` 서비스입니다. 공개 피드형 Pics와 달리 특정 날짜, 친구 또는 그룹, 시간 슬롯을 중심으로 하루의 조각을 모아 보여주는 기록형 소셜 기능으로 설계합니다.
  - **picLog 핵심 차별점:** Pics는 사용자가 고른 사진을 캡션과 함께 공개 피드에 올리는 콘텐츠이고, picLog는 정해진 하루 안에서 여러 사용자가 같은 로그에 사진을 채워 넣는 공동 기록입니다. picLog는 완성도 높은 게시물보다 지금의 순간, 참여 여부, 하루 흐름을 우선합니다.
  - **picLog 진입 구조:** 기본 경로는 `/snaps/pic-log`로 두고, 생성은 `/snaps/pic-log/new`, 상세는 `/snaps/pic-log/:logId`, 사진 추가는 `/snaps/pic-log/:logId/add`, 수정은 `/snaps/pic-log/:logId/edit`, 초대 입장은 `/snaps/pic-log/:logId/join` 경로를 사용합니다. 초대 링크는 바로 상세에 입장시키지 않고 참여자 여부와 패스워드 확인을 거치는 입장 화면으로 연결합니다. 실제 화면 폴더는 `src/pages/Snaps/PicLog/Index.tsx` 구조를 기준으로 합니다.
  - **picLog 로그 단위:** 하나의 로그는 `로그 제목`, `기록 날짜`, `참여자`, `공개 범위`, `초대 패스워드`, `시간 챕터`, `사진 항목`, `참여자 표시 순서`로 구성합니다. 제목은 사용자가 입력하되 비워둘 경우 날짜 기반으로 `2026년 5월 25일의 picLog`처럼 내부 생성할 수 있습니다.
  - **picLog 시간 챕터:** 시간 챕터는 아침 8시부터 저녁 10시까지 1시간 단위로 제공합니다. 기준 챕터는 `08:00`, `09:00`, `10:00`, `11:00`, `12:00`, `13:00`, `14:00`, `15:00`, `16:00`, `17:00`, `18:00`, `19:00`, `20:00`, `21:00`, `22:00`이며, 아무도 사진을 등록하지 않은 시간 챕터는 UI에 노출하지 않습니다. 비어 있는 날짜도 목록과 날짜 이동 UI에서 노출하지 않습니다.
  - **picLog 챕터 이동:** 시간 챕터 이동은 좌우 스와이프를 기본 인터랙션으로 사용하고 Embla Carousel 기반으로 구현합니다. 스와이프가 어려운 사용자를 위해 접근성 이름이 있는 이전/다음 `IconButton`을 보조로 제공할 수 있습니다. 챕터 변경 시 현재 챕터의 사진 영역과 댓글 영역이 함께 전환됩니다.
  - **picLog 날짜 이동:** 사용자는 이전 기록을 볼 수 있도록 날짜를 이동할 수 있습니다. 기본 조회 기간은 최근 14일을 권장하며, 오래된 기록을 자주 다시 보는 사용성을 고려해 2차에서 30일 또는 월별 아카이브로 확장할 수 있게 설계합니다. 날짜 이동 UI는 등록된 사진이 있는 날짜만 선택지로 보여줍니다.
  - **picLog 작성 흐름:** 사용자는 로그를 만들고 친구를 초대하거나 혼자 시작할 수 있습니다. 사진 추가 화면에서는 사진 1장, 짧은 메모, 현재 시간 챕터, 위치 공개 여부를 입력합니다. 한 번에 여러 장을 올리는 Pics와 달리 picLog는 순간 단위 사진 1장을 빠르게 올리는 흐름을 기본으로 합니다.
  - **picLog 5초 순간 영상 후보:** picLog의 기본 정체성은 사진 중심으로 유지하되, 추후 `5초 순간 영상`을 사진 추가 흐름의 선택 기능으로 확장할 수 있게 설계합니다. 당장 `/snaps/pic-log/:logId/add` 화면에 연결하지 않고, 먼저 재사용 가능한 독립 기능으로 `useTimedVideoRecorder` 훅과 `TimedVideoCapture` 컴포넌트를 만들 수 있게 준비합니다. 이 기능은 사용자가 명시적으로 `구현`을 요청한 뒤에만 코드로 작성하고, 완성 후에도 picLog 화면 연결 여부는 별도 확인을 받아 결정합니다.
  - **5초 영상 촬영 동작:** 촬영 기능은 `navigator.mediaDevices.getUserMedia()`로 모바일 후면 카메라를 우선 요청하고, `MediaRecorder`로 최대 5초만 녹화한 뒤 자동 종료합니다. 녹화 결과는 `Blob`, MIME 타입, 재생용 object URL, 길이, 파일명 후보를 반환하며, 호출자가 업로드 또는 미리보기를 결정합니다. 녹화 중 취소하면 recorder를 중지하고 생성한 object URL과 media stream track을 모두 정리합니다.
  - **5초 영상 브라우저 대응:** 카메라 직접 제어는 HTTPS와 사용자 권한 허용이 필수입니다. `MediaRecorder`가 없거나 iOS Safari처럼 특정 MIME 타입 지원이 제한되는 환경에서는 `input type="file" accept="video/*" capture="environment"` 대체 흐름을 제공합니다. 단, 파일 입력 대체 방식은 웹에서 5초 자동 종료를 보장할 수 없으므로, 선택한 파일의 길이를 후검증하거나 서버 처리 단계에서 자르는 정책을 별도로 둡니다.
  - **5초 영상 구현 파일 주석 기준:** 구현 시 `src/hooks/useTimedVideoRecorder.ts`, `src/components/molecules/TimedVideoCapture/index.tsx`, `src/assets/styles/components/TimedVideoCapture.scss`, 필요 시 `src/types/mediaCapture.ts`를 후보 파일로 둡니다. 각 파일에는 한글 주석으로 권한 요청 이유, 녹화 시작/자동 종료 흐름, `MediaRecorder` MIME 선택 이유, 스트림 track cleanup, object URL 해제, 미지원 브라우저 fallback의 한계를 설명합니다. 주석은 긴 설명문이 아니라 유지보수자가 실수하기 쉬운 생명주기와 브라우저 제약을 짚는 수준으로 제한합니다.
  - **picLog 사진 및 메모 수정:** 등록한 사진과 메모는 작성자 본인과 관리자만 수정할 수 있습니다. 사진 교체, 메모 수정, 시간 챕터 변경을 허용하되, 이미 댓글이 달린 항목은 변경 이력을 남길 수 있게 설계합니다. 수정 중 내용이 있으면 이탈 전 `Confirm`을 사용하고, 저장 성공은 `Toast`로 안내합니다.
  - **picLog 참여자 순서:** 로그에 입장하면 참여자별 사진 영역은 로그의 현재 `참여자 표시 순서`에 따라 정렬됩니다. 순서를 바꾸고 싶은 사용자는 직접 위치를 강제 변경하지 않고, 바꾸려는 위치의 사용자에게 `순서 변경 요청`을 보냅니다. 상대 사용자가 수락하면 두 사용자의 표시 순서를 교환하고, 거절하거나 만료되면 기존 순서를 유지합니다. 요청 중인 위치는 중복 요청을 막고, 수락/거절 결과는 Toast 또는 알림으로 안내합니다.
  - **picLog 사진 정책:** 사진은 `jpg`, `jpeg`, `png`, `webp`만 허용하고 장당 5MB 이하를 권장합니다. 업로드된 사진은 공통 `Img` 컴포넌트로 렌더링하며, 사진마다 `alt` 또는 대체 설명을 입력할 수 있게 합니다. 필터/보정/편집 기능은 제공하지 않습니다.
  - **picLog 공개 범위:** 1차 공개 범위는 `나만 보기`, `초대된 친구만`, `링크를 아는 사람`, `전체 공개` 후보를 둡니다. 기본값은 `초대된 친구만`으로 두어 친밀한 기록 감성을 우선하며, 전체 공개는 서비스 운영 정책과 신고/차단 기능이 준비된 뒤 활성화합니다. `초대된 친구만`과 `링크를 아는 사람` 범위는 초대 패스워드 확인을 기본 보안 장치로 사용합니다.
  - **picLog 초대 UI:** 상세 상단에는 초대 `IconButton`을 제공합니다. 버튼을 누르면 `Dialog` 팝업으로 초대 링크와 방 패스워드를 함께 보여주며, 각각 복사 버튼을 제공합니다. 패스워드는 방 생성 시 자동 생성하고 로그 생성자와 관리자만 재생성할 수 있습니다. 패스워드 재생성 시 기존 링크로 들어온 신규 사용자는 새 패스워드를 입력해야 하며, 이미 참여자인 사용자는 영향을 받지 않습니다. 1차 구현은 사용자가 다시 확인하고 전달할 수 있는 표시형 초대 패스워드로 저장하고, 서버 hook이 준비되면 원문 재표시 대신 재생성 중심의 해시 검증 구조로 강화합니다.
  - **picLog 초대 입장:** 초대 링크로 접근한 사용자가 이미 로그의 `participants`에 포함되어 있으면 패스워드 확인 없이 상세 화면으로 이동합니다. 참여자가 아니면 로그인 및 이메일 인증 상태를 먼저 확인하고, 통과 후 `/snaps/pic-log/:logId/join` 화면에서 패스워드를 입력받습니다. 패스워드가 일치하면 해당 사용자를 `participants`와 `participantOrder` 끝에 추가한 뒤 상세로 이동하고, 틀리면 필드 에러와 Toast로 안내합니다. 삭제되었거나 공개 권한이 없는 로그는 `ErrorState`를 보여줍니다.
  - **picLog 목록:** `/snaps/pic-log` 목록은 날짜별 로그 카드 중심으로 구성합니다. 카드에는 대표 사진 그리드, 로그 제목, 참여자 아바타, 사진 수, 마지막 업데이트 상대 시간, 공개 범위를 표시합니다. 필터는 `mine=1`, `date`, `visibility`, `keyword`를 URL query와 동기화합니다.
  - **picLog 상세:** 상세 화면은 시간 챕터 슬라이드를 기본 뷰로 사용합니다. 한 챕터 안에서는 참여자 표시 순서에 따라 사진을 정렬하고, 각 사진에는 작성자, 메모, 수정 가능 상태, 댓글 이동 버튼을 함께 제공합니다. 상단에는 날짜 이동, 참여자, 공유/초대 액션을 제공하고, 하단에는 사진 추가 액션과 댓글 입력 영역을 배치합니다. 초대 액션은 일반 공유와 구분하여 방 참여를 위한 링크와 패스워드 전달에 집중합니다.
  - **picLog 완성 뷰:** 하루가 끝난 로그는 `하루 모아보기` 뷰를 제공합니다. 대표 사진들을 시간 순서대로 압축해 보여주며, 저장 가능한 이미지 콜라주 또는 공유용 링크는 2차 기능으로 검토합니다. 영상 자동 생성은 1차 범위에서 제외합니다.
  - **picLog 반응:** picLog 전체에는 `좋아요`, `스크랩`, `공유하기`를 제공하고, 개별 사진에는 가벼운 이모지 반응 또는 좋아요를 확장 후보로 둡니다. 1차에서는 댓글보다 사진별 짧은 메모와 로그 전체 댓글을 우선합니다.
  - **picLog 댓글:** 댓글은 시간 챕터 단위로 달리는 형태를 기본으로 합니다. 댓글에는 `@nickname` 사용자 태그 권한을 제공하여 누구에게 말하는 댓글인지 명확히 표시할 수 있어야 합니다. 사진 영역의 댓글 아이콘 버튼을 누르면 하단 댓글 입력 영역으로 스크롤 이동하고, 입력값 앞에 해당 사진 작성자의 닉네임 태그가 자동 삽입됩니다. 태그 대상은 같은 로그의 참여자 또는 해당 챕터에 사진을 등록한 사용자로 제한하며, 닉네임은 전역 유니크 값으로만 태그를 매칭합니다. 대댓글은 기존 댓글 정책과 동일한 `parentComment` 관계로 확장할 수 있게 합니다.
  - **picLog 권한:** 로그 생성, 사진 추가, 반응, 댓글 작성은 로그인 및 이메일 인증 완료 회원만 가능합니다. 조회 권한은 공개 범위에 따르며, 초대형 로그는 참여자와 초대 패스워드를 통과한 사용자만 접근할 수 있어야 합니다. 수정/삭제는 로그 생성자, 사진 작성자, 관리자 권한을 분리합니다. 초대 입장은 PocketBase update rule에서 `invitePassword`와 요청 본문의 패스워드를 비교해 검증하고, 검증이 통과한 사용자만 참여자 목록에 추가합니다.
  - **picLog 삭제 정책:** 로그 삭제는 생성자와 관리자만 가능하며 삭제 전 `Confirm`을 사용합니다. 사진 삭제는 해당 사진 작성자, 로그 생성자, 관리자만 가능하고 soft delete로 처리합니다.
  - **picLog 알림:** 1차 구현은 앱 내부 Toast/상태 안내 중심으로 두고, 리마인드 알림은 브라우저 알림 권한, PWA 여부, 서버 푸시 정책이 확정된 뒤 별도 설계합니다.
  - **picLog Snaps 서브 메인 노출:** Snaps 서브 메인에는 `Pics`와 `picLog`를 서로 다른 카드로 노출합니다. Pics는 공개 이미지 피드, picLog는 친구와 함께 채우는 하루 사진 로그로 설명하고, 최근 picLog와 오늘 진행 중인 picLog를 우선 보여줍니다.
- **소셜 인터랙션:**
  - **좋아요/싫어요(Reaction):** 게시글, 댓글, Clips, Clips 댓글에 좋아요와 싫어요 기능을 제공합니다. 같은 사용자는 같은 대상에 좋아요와 싫어요 중 하나만 선택할 수 있으며, 다시 누르면 기존 반응 레코드를 삭제해 취소합니다. 좋아요에서 싫어요로 전환하거나 반대로 전환할 때는 기존 반응을 삭제한 뒤 새 반응을 생성합니다. `reactions`는 토글 상태 데이터이므로 다른 컬렉션에서 필수 relation으로 참조하지 않고, 알림이나 활동 로그는 `actor`, `targetType`, `targetId`, `type` 같은 스냅샷 값으로 반응 사실을 저장합니다. 중복 방지, 낙관적 업데이트, 실패 시 rollback, 실시간 카운트 반영을 기본 정책으로 둡니다.
...
- **Spinner:** 짧은 로딩, 버튼 내부 로딩, 리스트 추가 로딩, 작은 위젯 진행 상태에 사용합니다.
- **PageLoading:** Anoju 로고, 회전 orbit, glow/pulse 같은 인터랙션을 포함한 페이지 차단 로딩 표시 컴포넌트입니다. 실제 화면에는 루트의 `PageLoadingOverlay`가 한 번만 렌더링하며, 각 화면에서 직접 렌더링하지 않습니다.
...
- **PageLoading 역할:** `PageLoading`은 로고 기반 인터랙션을 가진 전역/페이지 단위 로딩 표시이고, 루트의 `PageLoadingOverlay`에서만 사용합니다. 화면/비동기 로직에서는 `openPageLoading`, `closePageLoading`, `usePageLoadingEffect`만 사용합니다.
- **중복 방지:** 전역 로딩 DOM은 하나만 노출합니다. 여러 로직이 동시에 `openPageLoading`을 호출하면 내부 카운터를 증가시키고, 같은 횟수만큼 `closePageLoading`이 호출되어야 닫힙니다. 특정 작업을 추적해야 하면 `openPageLoading`이 반환한 id로 닫고, 강제 전체 종료가 필요하면 `closePageLoading({ all: true })`를 사용합니다.
- **적용 대상:** 전역 페이지 로딩은 인증 상태 확인, 보호 라우트 판정, 목록 첫 조회, 상세/수정 화면의 필수 레코드 조회, 관리자 초기 설정 조회처럼 화면을 구성할 데이터가 아직 없는 경우에 사용합니다.
- **비적용 대상:** 버튼 저장/삭제/업로드, 좋아요/댓글 제출, 무한스크롤 다음 페이지, 리스트 더보기, 이미 표시된 데이터의 재조회, 비디오 플레이어 내부 버퍼링, 소셜 로그인 provider 영역처럼 화면 전체를 막지 않아도 되는 진행 상태에는 전역 페이지 로딩을 사용하지 않습니다.
...
테스트 계정은 실제 사용자와 구분할 수 있도록 닉네임에 테스트 계정임을 드러내고, 게시글/댓글/갤러리 작성 검증에만 사용합니다.
...
- **reactions:** 좋아요/싫어요 반응
  ...
  - `type`: `like | dislike`
  - 반응 취소와 전환은 기존 레코드 삭제를 전제로 하므로 `reactions`를 다른 컬렉션의 required relation으로 참조하지 않습니다.
- **bookmarks:** 게시글 스크랩
  - `post`, `user`
- **reports:** 신고
...
- **내 게시물 필터:** 게시판/갤러리 목록의 `내 게시물 보기`는 query string으로 상태를 보존합니다. 예: `/lounge/free-board?mine=1`, `/snaps/pics?mine=1`. `mine=1`이 활성화되면 현재 로그인한 사용자 id를 작성자 조건으로 사용하고, 미로그인 상태에서는 로그인 화면으로 이동하며 redirect query로 원래 경로를 보존합니다.
- **디바이스정보 필터:** `/lounge/device?manufacturer=Samsung&model=Galaxy&keyword=fold`처럼 제조사, 모델, 검색어를 query string으로 보존합니다. 제조사 변경 시 모델 필터는 초기화하고, 필터 변경 시 페이지 또는 커서를 초기화합니다.
...
- **수정 권한:** 자유게시판, Pics, 디바이스정보 수정은 작성자 본인과 관리자만 사용할 수 있습니다. 프론트는 버튼 노출을 제한하고 PocketBase API Rules로 최종 권한을 검증합니다.
...
- **게시물 삭제 권한:** 자유게시판, Pics, 디바이스정보 게시물 삭제는 작성자 본인과 관리자만 사용할 수 있습니다. 작성자 삭제는 soft delete로 숨김 처리하고, hard delete는 관리자만 사용할 수 있습니다.
...
- **Clips 동영상:** 1차 업로드 정책은 `mp4`, `webm`, `mov` 형식, 300MB 이하를 기본 입력 제한으로 두고, 저장본은 사용자가 선택한 30초 이하 구간을 720p 이하 MP4로 변환합니다. 720p 판정은 가로/세로 방향을 고려해 긴 변 1280px 이하, 짧은 변 720px 이하를 기본 프로필로 사용합니다.
- **picLog 5초 순간 영상 후보:** picLog에 연결할 수 있는 짧은 영상은 사진 흐름의 보조 기능으로 두며, 클라이언트 직접 촬영은 5초 자동 종료, 파일 입력 fallback은 길이 후검증을 기준으로 합니다. 입력 후보 형식은 `webm`, `mp4`, `mov`로 두되, 브라우저별 `MediaRecorder` 출력 포맷 차이를 고려해 서버 저장 포맷과 클라이언트 촬영 MIME을 분리해서 검증합니다.
- **동영상 업로드 옵션화:** 동영상 제한값은 화면에 직접 하드코딩하지 않고 재사용 가능한 업로드 프로필과 공통 `VideoUploadField` 컴포넌트 옵션으로 관리합니다. 추후 화면별로 길이, 용량, 해상도 정책이 달라질 수 있음을 전제로 합니다.
- **동영상 최종 처리:** 클라이언트는 업로드 전 메타데이터로 구간 선택 UI를 제공하고, 서버/PocketBase 훅 또는 별도 처리 파이프라인에서 ffmpeg 기반 자르기, 720p 변환, 최종 메타데이터 검증을 수행합니다.
...
- **우측 영역:** `우측 확장 영역` → `알림 버튼` → `홈 버튼` 순서로 배치합니다.
- **알림 버튼:** 홈 화면의 메인 헤더 우측에는 알림함으로 이동하는 `IconButton`을 제공합니다. 비로그인 사용자가 누르면 보호 라우트를 통해 로그인 화면으로 이동하고, 로그인 사용자는 알림함으로 이동합니다. 읽지 않은 알림이 있으면 접근성 이름과 시각 배지를 함께 갱신합니다. 알림 버튼은 기본적으로 홈 화면에서만 노출하고, 노출 여부는 라우트/레이아웃 설정에서 화면별로 제어할 수 있어야 합니다.
...
- **플로팅 목적지 노출:** `전체메뉴`, `lounge`, `Snaps`, `마이페이지` 또는 로그인 전 `로그인`처럼 플로팅 메뉴로 이동하는 화면은 기본적으로 플로팅 메뉴를 노출합니다.
...
- **기본 메뉴 구성:** 하단 플로팅메뉴의 기본 항목은 `전체메뉴`, `lounge`, `Snaps`, `마이페이지`로 구성하며, 기존 `홈` 항목은 제외합니다.
- **메뉴 목적:** `lounge`는 커뮤니티형 게시판 묶음의 1뎁스 서브 메인으로 이동하고, `Snaps`는 시각 콘텐츠 영역의 1뎁스 서브 메인으로 이동합니다.
- **Snaps 진입:** 갤러리형 콘텐츠 묶음의 1뎁스 메뉴명과 플로팅 항목명은 `Snaps`로 통일합니다. 플로팅 메뉴에서 `Pics`로 바로 이동하지 않습니다.
...
- **알림함 라우트:** `/my-page/notifications`는 `requiresAuth: true`, `robots: 'noindex'`로 관리하고, 헤더 타이틀은 `알림함`으로 표시합니다. 메인 헤더의 알림 버튼은 이 경로로 이동합니다.
