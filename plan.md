...
- **피드백:** `Toast`, `Dialog`, `Alert`, `Confirm`, `BottomSheet`, `Spinner`, `GlobalLoading`, `PageLoading`, `EmptyState`, `ErrorState`
- **데이터 표시:** `DataList`
- **미디어:** `Img`
...
- **GlobalLoading:** Anoju 로고 기반 전역 로딩 시각 컴포넌트입니다. 로고, 회전 orbit, glow/pulse 같은 인터랙션을 포함하며, 직접 화면에서 남발하지 않고 `PageLoading`이 호출합니다.
- **PageLoading:** `GlobalLoading`을 감싼 페이지 차단 로딩 래퍼입니다. 인증 초기화, 필수 라우트 데이터 확인, 상세/수정 화면의 첫 데이터 조회처럼 페이지 진입 자체를 막아야 할 때만 제한적으로 사용합니다.
...
- **GlobalLoading 역할:** `GlobalLoading`은 로고 기반 인터랙션을 가진 시각 컴포넌트이고, 일반 화면에서는 `PageLoading`을 통해서만 사용합니다. 직접 `GlobalLoading`을 호출하는 경우는 시연/프리뷰 또는 전역 앱 오버레이처럼 명확한 예외로 제한합니다.
- **적용 대상:** `PageLoading`은 인증 상태 확인, 보호 라우트 판정, 목록 첫 조회, 상세/수정 화면의 필수 레코드 조회, 관리자 초기 설정 조회처럼 화면을 구성할 데이터가 아직 없는 경우에 사용합니다.
- **비적용 대상:** 버튼 저장/삭제/업로드, 좋아요/댓글 제출, 무한스크롤 다음 페이지, 리스트 더보기, 이미 표시된 데이터의 재조회, 비디오 플레이어 내부 버퍼링, 소셜 로그인 provider 영역처럼 화면 전체를 막지 않아도 되는 진행 상태에는 `PageLoading`을 사용하지 않습니다.
