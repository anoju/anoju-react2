...
- **컨텐츠 레이아웃:**
  ...
  - **picLog 진입 구조:** 기본 경로는 `/snaps/pic-log`로 두고, 생성은 `/snaps/pic-log/new`, 상세는 `/snaps/pic-log/:logId`, 사진 추가는 `/snaps/pic-log/:logId/add`, 수정은 `/snaps/pic-log/:logId/edit` 경로를 사용합니다. 실제 화면 폴더는 `src/pages/Snaps/PicLog/Index.tsx` 구조를 기준으로 합니다.
  - **picLog 로그 단위:** 하나의 로그는 `로그 제목`, `기록 날짜`, `참여자`, `공개 범위`, `시간 챕터`, `사진 항목`, `참여자 표시 순서`로 구성합니다. 제목은 사용자가 입력하되 비워둘 경우 날짜 기반으로 `2026년 5월 25일의 picLog`처럼 내부 생성할 수 있습니다.
  ...
  - **picLog 공개 범위:** 1차 공개 범위는 `나만 보기`, `초대된 친구만`, `링크를 아는 사람`, `전체 공개` 후보를 둡니다. 기본값은 `초대된 친구만`으로 두어 친밀한 기록 감성을 우선하며, 전체 공개는 서비스 운영 정책과 신고/차단 기능이 준비된 뒤 활성화합니다.
  ...
  - **picLog 상세:** 상세 화면은 시간 챕터 슬라이드를 기본 뷰로 사용합니다. 한 챕터 안에서는 참여자 표시 순서에 따라 사진을 정렬하고, 각 사진에는 작성자, 메모, 수정 가능 상태, 댓글 이동 버튼을 함께 제공합니다. 상단에는 날짜 이동, 참여자, 공유/초대 액션을 제공하고, 하단에는 사진 추가 액션과 댓글 입력 영역을 배치합니다.
  ...
  - **picLog 권한:** 로그 생성, 사진 추가, 반응, 댓글 작성은 로그인 및 이메일 인증 완료 회원만 가능합니다. 조회 권한은 공개 범위에 따르며, 초대형 로그는 참여자와 초대된 사용자만 접근할 수 있어야 합니다. 수정/삭제는 로그 생성자, 사진 작성자, 관리자 권한을 분리합니다.
...
- **알림 버튼:** 로그인한 사용자에게는 헤더 우측에 알림함으로 이동하는 `IconButton`을 제공하고, 읽지 않은 알림이 있으면 접근성 이름과 시각 배지를 함께 갱신합니다. 알림 버튼 노출 여부는 라우트/레이아웃 설정에서 화면별로 제어할 수 있어야 합니다.
