...
- **Clips 동영상:** 1차 업로드 정책은 `mp4`, `webm`, `mov` 형식, 100MB 이하, 30초 이하, 최대 720p 기준으로 제한합니다. 720p 판정은 가로/세로 방향을 고려해 긴 변 1280px 이하, 짧은 변 720px 이하를 기본 프로필로 사용합니다.
...
- **동영상 최종 검증:** 클라이언트는 업로드 전 메타데이터로 길이와 해상도를 1차 검증하지만, 서버/PocketBase 훅 또는 별도 처리 파이프라인에서 동일 정책을 최종 검증해야 합니다.
