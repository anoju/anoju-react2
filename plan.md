...
- **메뉴 구조:**
  ...
  - `playground` 하위 2뎁스에는 모바일 웹 해상도 확인용 `디바이스정보` 게시판을 함께 둡니다.
  ...
  - 플로팅 메뉴의 `playground`와 `Snaps` 항목은 각각 1뎁스 서브 메인 화면으로 이동해야 하며, 2뎁스 목록 또는 상세 화면으로 바로 이동하지 않습니다.
- **컨텐츠 레이아웃:**
  ...
  - **자유게시판 수정:** `/playground/free-board/:postId/edit` 경로에서 제목과 본문을 수정합니다. 수정 권한은 작성자 본인과 관리자에게만 제공하며, 저장 성공 후 상세 화면으로 이동합니다.
  ...
  - **디바이스정보 수정:** `/playground/device/:deviceReportId/edit` 경로에서 제조사, 모델, 스크린 사이즈, 윈도우 사이즈, ratio, 스크린방향, userAgent, 디스플레이설정, 추가설명을 수정합니다. 자동으로 등록된 측정값도 직접 입력할 수 있고, 윈도우 사이즈는 현재 브라우저 기준으로 다시 측정하는 액션을 제공합니다. 수정 권한은 작성자 본인과 관리자에게만 제공합니다.
...
- **내 게시물 필터:** 게시판/갤러리 목록의 `내 게시물 보기`는 query string으로 상태를 보존합니다. 예: `/playground/free-board?mine=1`, `/snaps/pics?mine=1`. `mine=1`이 활성화되면 현재 로그인한 사용자 id를 작성자 조건으로 사용하고, 미로그인 상태에서는 로그인 화면으로 이동하며 redirect query로 원래 경로를 보존합니다.
- **디바이스정보 필터:** `/playground/device?manufacturer=Samsung&model=Galaxy&keyword=fold`처럼 제조사, 모델, 검색어를 query string으로 보존합니다. 제조사 변경 시 모델 필터는 초기화하고, 필터 변경 시 페이지 또는 커서를 초기화합니다.
...
- **플로팅 목적지 노출:** `전체메뉴`, `playground`, `Snaps`, `마이페이지` 또는 로그인 전 `로그인`처럼 플로팅 메뉴로 이동하는 화면은 기본적으로 플로팅 메뉴를 노출합니다.
...
- **기본 메뉴 구성:** 하단 플로팅메뉴의 기본 항목은 `전체메뉴`, `playground`, `Snaps`, `마이페이지`로 구성하며, 기존 `홈` 항목은 제외합니다.
- **메뉴 목적:** `playground`는 커뮤니티형 게시판 묶음의 1뎁스 서브 메인으로 이동하고, `Snaps`는 시각 콘텐츠 영역의 1뎁스 서브 메인으로 이동합니다.
