...
- **컨텐츠 레이아웃:** 
  ...
  - **자유게시판 목록:** 모바일 중심의 단순 리스트 형태로 구성합니다. 제목, 작성자, 작성일, 댓글 수, 조회 수, 좋아요 수를 빠르게 스캔할 수 있게 하되, 테이블형 레이아웃은 사용하지 않습니다.
  - **자유게시판 에디터:** 글 중간에 이미지를 첨부할 수 있는 리치 텍스트 에디터를 사용합니다. 이미지는 본문 흐름 안에 삽입되며, 별도 파일 업로드 정책과 `Img` 렌더링 정책을 따릅니다.
  - **자유게시판 댓글:** 댓글과 대댓글을 지원하되, 작성은 로그인 및 이메일 인증 완료 회원만 가능하고 조회는 모든 사용자가 가능합니다.
  - **Pics:** 인스타그램 같은 소셜 피드 UI를 기준으로 이미지가 중심이 되는 카드형 피드를 제공합니다.
  - **Pics 피드:** 작성자, 이미지, 캡션, 좋아요, 댓글, 스크랩, 공유 액션을 한 흐름에서 확인할 수 있게 구성합니다.
  - **Pics 이미지 정책:** 이미지 필터, 보정, 편집 기능은 제공하지 않습니다. 업로드, 순서 관리, 대표 이미지 지정, 확대 보기 중심으로 제한합니다.
  - **Pics 상세:** 피드에서 자연스럽게 이어지는 상세 화면을 제공하고, 댓글은 하단 또는 별도 섹션으로 접근성을 유지합니다.
- **소셜 인터랙션:**
