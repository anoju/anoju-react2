...
- **컨텐츠 레이아웃:** 
  ...
  - **내 게시물 보기:** 자유게시판, Pics를 포함한 모든 게시판/갤러리 목록에는 `내 게시물 보기` 기능을 제공합니다. 로그인한 사용자의 작성글만 현재 목록 안에서 필터링하며, 마이페이지의 `내가 작성한 글`처럼 전체 도메인을 모아 보여주는 화면과는 별도 기능으로 구분합니다. 미로그인 상태에서 사용하면 로그인 화면으로 이동하고, 로그인 후 기존 목록 경로와 query로 복귀할 수 있어야 합니다.
  ...
  - **디바이스정보 디스플레이설정:** Android는 설정 단계 숫자를 직접 입력할 수 있고, 가장 왼쪽 단계를 `1`로 보고 현재 단계를 입력한다는 설명을 함께 제공합니다. iPhone/iPad/iOS 계열은 `0`으로 자동 입력하고 수정할 수 없습니다.
  ...
  - **디바이스정보 수정:** `/playground/device/:deviceReportId/edit` 경로에서 제조사, 모델, 디스플레이설정, 추가설명을 수정합니다. 스크린/윈도우/ratio/방향/userAgent는 작성 당시 측정값 보존을 기본으로 하며, 수정 권한은 작성자 본인과 관리자에게만 제공합니다.
